...
const contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    emailjs.send('service_04w06kc', 'template_ilvsmlq', {
      from_name: form.name,
      to_name: 'Ayush',
      from_email: form.email,
      to_email: 'master.ayush.2003@gmail.com',
      message: form.message,
    }, 'WjhMLUKI6m8HGnM8x').then(() => {
      setLoading(false);
  ...
  return (
    <div className='xl:mt-12 xl:flex-row flex-col-reverse flex gap-10 overflow-hidden py-20'>
      <motion.div
